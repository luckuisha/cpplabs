...
private:
   ...
   //insertion
   bool helpInsert (DBentry* newEntry, TreeNode* parent, string key);
   //search
   DBentry* helpFind(TreeNode * ptr, string key);
   //clear
   void helpClear(TreeNode* parent);
   //count reference to int cnt which allows for the same value of cnt to be incremented
   int helpCount(TreeNode * parent, int & cnt) const;
   //delete
   bool helpDelete(TreeNode * parent, string key);
   //find right most treenode and return pointer to it
   TreeNode * findRight (TreeNode * parent);
   //print
   void helpPrint (ostream& out,TreeNode * print) const;
